...
public class MakeLogEntry
{
  ...
  String LOGBOOKNAME="HDLOG";

    String[] LOGBOOKS = {"HDLOG", "HDBCAL", "HDCCAL", "HDCDC", "HDCPS", "HDFCAL", "HDFCAL2", "HDFDC", "HDFMWPC", "HDDIRC", "HDGAS", "HDJEF", "HDMONITOR", "HDRUN", "HDSAD", "HDSOLENOID", "HDSTARTCOUNTER", "HDTAGGER", "HDTARGET", "HDTOF", "HDTRIGGER", "TLOG"};

    // final String RUNDBTABLE="daq_"+System.getenv("USER");
    // final String RUNDBTABLE="rcdb";
  final String RUNDBTABLE="rcdb2";
    //  final String RUNDBUSER=System.getenv("EXPID");
  final String RUNDBUSER="rcdb";
    //final String RUNDBUSER="rcdb2";
  final String RUNDBSESSION=System.getenv("SESSION");
    //  final String RUNDBHOST=System.getenv("MYSQL_HOST");
  final String RUNDBHOST="gluondb1.jlab.org";
  final String RUNDBPORT="3306";
  final String RUNDBPASSWD="GlueX_2come";
...
//  String RUNDBUSER=RUNDBEXPID;
  ...
  public int getRunNumber()
  {
    ...
    try {
      ...
      String query = "select number from runs order by number desc limit 1 ;";
      ...
      while (rs.next()) runNumber=rs.getInt("number");
...
//      if (count>10) System.err.println("MakeLogEntry.java:  WAITCOUNT2:  "+count);
  ...
  {
    LOGTITLE.setText("HallD Run #"+Integer.toString(getRunNumber())+":  ");
    LOGTITLE.updateUI();
